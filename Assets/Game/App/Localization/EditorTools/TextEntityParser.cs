...
namespace Game.Localization.UnityEditor
{
    ...
    {
        public static TextEntity[] ParseTextByEntities(string text)
        {
            ...
            {
                return new TextEntity[0];
            }

            var headerLine = lines[0];
            var languages = ParseHeaderByLanguages(headerLine);

            var entities = new TextEntity[lineCount - 1]; //Without header line:
        ...
        public static TextEntity ParseEntityByLine(string line, SystemLanguage[] languages)
        {
            var chunks = ParseLine(line);
            var key = chunks[0];
            
            var translationCount = languages.Length;
            var translations = new LocalizedString[translationCount];
